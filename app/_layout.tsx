...
export default function RootLayout() {
  ...
  useEffect(() => {
    // Оборачиваем инициализацию в try-catch для безопасности
    try {
      // Используем setTimeout для предотвращения блокировки основного потока
      setTimeout(() => {
        try {
          initializeServices()
          ...
          // Продолжаем работу приложения несмотря на ошибки
        }
      }, 0)
    } catch (error) {
      console.error('Critical error during initialization setup:', error)
      // Этот внешний обработчик защищает от любых исключений
    }
  }, [])
