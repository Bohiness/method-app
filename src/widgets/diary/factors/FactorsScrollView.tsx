...
export function FactorsScrollView({ selectedFactors, onSelect, iconView = false, search = false }: FactorsScrollViewProps) {
            ...
            {!iconView ? (
                ...
                <View className="flex-col gap-y-6">
                    {groupedFactors?.map((group, groupIndex) => (
                        <View key={groupIndex} className="flex-row justify-between">
                            {group.map((factor) => (
                                <IconButtonWithText
                                    key={factor.id}
                                    onPress={() => handleSelect(factor.id)}
