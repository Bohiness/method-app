...
export const usePasswordReset = ({
    ...
    const verifyCode = async (code: string) => {
        setLoading(true);
        try {
            const response = await apiClient.post<{ message: string; token: string }>(API_ROUTES.AUTH.CHECK_CODE, {
                email,
                code,
            });
    ...
    const resetPassword = async (password: string, password2: string) => {
        ...
        try {
            const response = await apiClient.post<{ message: string }>(API_ROUTES.AUTH.SET_PASSWORD, {
                email,
                password,
                password2,
                token,
