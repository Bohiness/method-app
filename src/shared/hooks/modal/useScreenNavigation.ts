...
export const useScreenNavigation = <T extends ScreenType>(initialScreen: T) => {
    ...
    const navigate = useCallback((screen: T) => {
    ...
    const previous = stack.length > 1 ? stack[stack.length - 2] : (history.length > 0 ? history[history.length - 1] : undefined);
