...
export function useLogger(componentName: string, props?: any, options: UseLoggerOptions = {}) {
  ...
  return {
    ...
    error: (message: any, error?: Error, title?: string) => 
      logger.error(message, componentName, error, title),
    table: (data: any[], options = {}) => 
      logger.table(data, { ...options, context: componentName }),
    json: (data: any, options = {}) => 
      logger.json(data, { ...options, context: componentName }),
    group: (label: string, callback: () => void) => logger.group(label, callback),
    api: (method: string, url: string, data?: any, response?: any) => 
      logger.api(method, url, data, response)
  };
}