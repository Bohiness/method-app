...
class AnonymousUserService {
  ...
  private async generateDeviceId(): Promise<string> {
    try {
      ...
      const deviceId = `${uniqueId}-${deviceName}-${modelName}`
        .replace(/\s+/g, '-')
        .toLowerCase();
...
async createAnonymousUser(): Promise<AnonymousResponse> {
    try {
        ...
        const response = await apiClient.post<AnonymousResponse>(
            API_ROUTES.AUTH.ANONYMOUS.CREATE,
            deviceInfo
        );
  ...
  async getOrCreateAnonymousUser(): Promise<AnonymousResponse> {
    try {
      ...
      if (existingUser && session) {
        console.debug('AnonymousService: Found existing anonymous user');
  ...
  async convertToRegisteredUser(data: Partial<UserType>): Promise<AnonymousResponse> {
    try {
      ...
      const response = await apiClient.post<AnonymousResponse>(
        API_ROUTES.AUTH.ANONYMOUS.CONVERT,
        {
          ...data,
