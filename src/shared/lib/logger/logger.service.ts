...
class Logger {
    ...
    async getFileLogContent(): Promise<string> {
        const fileTransport = this.transports.find(
          t => t instanceof FileTransport
        ) as FileTransport;
  ...
  private formatLogMessage(type: string, message: any, context?: string, title?: string): string {
    ...
    const parts = [
      this.applyStyle(timestamp, { dim: true }),
      this.applyStyle(type.padEnd(7), { color: '36' })
    ];
  ...
  async getLogs(): Promise<LogData[]> {
    ...
    return await storage.get<LogData[]>(storageTransport.config.key) || [];
  }
    ...
    for (const transport of this.transports) {
      try {
        ...
        states[transport.constructor.name] = {
          ...
          levels: transport.config.levels
        };
      } catch (error) {
        states[transport.constructor.name] = { error: error.message };
      }
    }
